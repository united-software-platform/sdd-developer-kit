...
# Перечень путей поставки задан переменной этого скрипта: установщик — его единственный
# читатель, и отдельный файл добавил бы в корень репозитория носитель, описывающий поставку.
# Пути берутся из распакованного архива, а не рядом с установщиком: состав поставки принадлежит
...
#
# Проверка объёма секций Makefile входит в поставку вместе с rules/: документ rules/makefile-rules.md
# предписывает её прогон, и правило без проверки оставило бы целевой проект с нормой, которую нечем
# проверить. Пайплайн .github/workflows/makefile-lint.yml в поставку не входит: сценарии системы
# непрерывной интеграции целевой проект ведёт сам, и обязательным для него остаётся локальный прогон.
#
# Значение обходится словоделением: пути поставки пробелов не содержат, и путь с пробелом
# в перечень не добавляется — иначе он молча разъедется на два.
PAYLOAD_PATHS="
docker-compose.yml
Makefile
.env.example
