...
#
# Файл — исходник образа репозитория kit'а, а не поставки: собирает и публикует образ
# пайплайн .github/workflows/docker-agent.yml, установленный проект получает готовый
# образ из реестра. Локальная сборка остаётся для разработки самого образа:
#
...
# Выпуск базового образа объявлен до FROM по той же причине. Значение передаётся снаружи:
# пайплайн разрешает подвижный latest в конкретный выпуск до сборки, иначе собранный образ
# нечем назвать — ни сверить состав с уже опубликованным, ни записать его в метки.
# Значение по умолчанию оставлено для локальной сборки: она не должна зависеть от опроса реестра.
...
FROM ghcr.io/united-software-platform/claude:${CLAUDE_VERSION}

# Версия передаётся снаружи: пайплайн разрешает её при сборке в пределах мажорного разряда,
# чтобы плановая пересборка приносила новые выпуски утилиты без правки этого файла. Значение
# по умолчанию оставлено для локальной сборки — она не должна зависеть от опроса npm-реестра.
# Мажорный разряд закреплён и здесь, и в пайплайне: несовместимая мажорная версия ломает
# скиллы openspec-* из .claude/skills, поэтому смена разряда — осознанная правка обоих мест.
ARG OPENSPEC_VERSION=1.11.0

# Версия Python закреплена полностью: интерпретатор задаёт среду выполнения проекта,
# и его смена — осознанная правка, а не побочный результат пересборки. Берётся он не из apt:
